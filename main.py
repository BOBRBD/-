...
# === ИСПРАВЛЕННЫЙ БЛОК ЗАГРУЗКИ .env ===
def load_bot_token():
    """Надежная загрузка токена из .env файла с обработкой BOM"""
    env_path = os.path.join(os.path.dirname(__file__), '.env')
    
    if not os.path.exists(env_path):
        raise FileNotFoundError(f"Файл .env не найден в папке: {os.path.dirname(__file__)}")
    
    # Способ 1: Чтение файла напрямую с обработкой BOM
    for encoding in ['utf-8-sig', 'utf-8', 'latin-1']:
        try:
            with open(env_path, 'r', encoding=encoding) as f:
                content = f.read()
                # Ищем BOT_TOKEN в содержимом
                for line in content.splitlines():
                    line = line.strip()
                    if line.startswith('BOT_TOKEN='):
                        # Берем всё после первого '='
                        token = line.split('=', 1)[1].strip()
                        # Удаляем возможные кавычки
                        token = token.strip('"\'').strip()
                        if token:
                            print(f"✅ Токен загружен из .env ({encoding}): {token[:10]}...")
                            return token
        except UnicodeDecodeError:
            continue
    
    # Способ 2: Через dotenv (если установлен)
    try:
        ...
        if token:
            token = token.strip('"\'').strip()
            print(f"✅ Токен загружен через dotenv: {token[:10]}...")
            return token
    except ImportError:
        pass
    ...
    raise ValueError(
        ...
        "Убедитесь, что файл .env содержит строку:\n"
        "BOT_TOKEN=8317308639:AAGyUDfnvu-IXfaVNEw7JIC6KihQeTvkI1Q\n\n"
        f"Текущая папка: {os.getcwd()}\n"
        f"Содержимое .env: {repr(open(env_path, 'rb').read()[:100])}"
    )
...
db = BirthdayDatabase()
active_users = set()  # Хранилище активных пользователей
...
async def send_birthday_reminders():
    ...
    while True:
        try:
            ...
            await asyncio.sleep(3600)  # 60 * 60 = 3600 секунд (1 час)
